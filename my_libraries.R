...
`%+%` <- function(x, y) unlist(lapply(seq_along(x), function(i) paste(x[i], y[i], collapse = "_", sep = "_")))

matchu <- function(x) return(match(u(x), x))
...
generate_DCR_pse <- function(wd = "./", gbucket = "gs://talk-iw-gcnv-working-dir/000-temp/", sample_set_entity = "sample_set_entity.tsv", participant = "participant"){
  df_sse <- fread(paste0(wd, sample_set_entity)) %>% as.data.frame()
  df_sse$batch <- df_sse$`entity:sample_set_id` %>% str_remove("(_CASE)|(_COHORT)")
  c_pse <- data.frame(`entity:participant_id` = df_sse$batch %>% u,
                      FILE_DCR = "",
                      FILE_SAMPLES = "",
                      check.names = FALSE)
  for(b in unique(df_sse$batch)){
    files_dcr <- df_sse$denoised_copy_ratios[df_sse$batch==b] %>% str_extract_all("gs:.*?tsv") %>% unlist
    output_name <- paste0("FILE_DCR_", b, ".txt")
    write.table(files_dcr, paste0(wd, output_name), sep="\t", col.names = FALSE, row.names = FALSE, quote=FALSE)
    system2("gsutil", paste0("cp ", wd, output_name, " ", gbucket, output_name))
    c_pse$FILE_DCR[c_pse$`entity:participant_id` == b] <- paste0(gbucket, output_name)
    
    vect_samples <- files_dcr %>% str_extract("(?<=denoised_copy_ratios-).*(?=.tsv)")
    output_name <- paste0("FILE_SAMPLES_", b, ".txt")
    write.table(vect_samples, paste0(wd, output_name), sep="\t", col.names = FALSE, row.names = FALSE, quote=FALSE)
    system2("gsutil", paste0("cp ", wd, output_name, " ", gbucket, output_name))
    c_pse$FILE_SAMPLES[c_pse$`entity:participant_id` == b] <- paste0(gbucket, output_name)
  }
  colnames(c_pse)[1] <- colnames(c_pse)[1] %>% str_replace("participant", participant)
  write.table(c_pse, paste0(wd, "c_pse_dcr.tsv"), sep="\t", col.names = TRUE, row.names = FALSE, quote=FALSE)
}
